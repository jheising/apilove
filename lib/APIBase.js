...
function APIEndpoint(options) {
    return function (target, key, descriptor) {
        ...
        let handlerData = lodash_1.get(targetStorage, `__handlerData.${key.toString()}`, {});
        ...
        lodash_1.set(targetStorage, `__handlerData.${key.toString()}`, handlerData);
    };
}
...
class APIBase {
    constructor() {
        ...
        let staticHandlers = lodash_1.get(this, "constructor.__handlerData");
        let instanceHandlers = lodash_1.get(this, "constructor.prototype.__handlerData");
        let processHandler = (handlerData, thisObject) => {
        ...
        lodash_1.each(staticHandlers, (handlerData) => processHandler(handlerData, this.constructor));
        lodash_1.each(instanceHandlers, (handlerData) => processHandler(handlerData, this));
    }
    _createHandlerWrapperFunction(handlerData, thisObject) {
        return (req, res) => {
